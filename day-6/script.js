...
function rendered(data) {
  ...
  data.forEach((items) => {
    ...
    title.classList.add("text-[white]", "font-bold", "text-[20px]");

    const author = document.createElement("p");
    author.classList.add("text-[white]", "font-bold", "text-[16px]");
    const year = document.createElement("p");
    year.classList.add("text-[white]", "font-bold", "text-[16px]");
...
find.addEventListener("click", () => {
  const filterdTitle = resources.filter(
    (items) => items.category === inputVal.value,
  );
  rendered(filterdTitle);
});
...
filterBtn.addEventListener("click", () => {
  const filtered = resources.filter((item) => item.category === categories);
  rendered(filtered);
});
